...
import { Loader2, Calendar, CheckCircle2, Search, RotateCcw, Clock, ChevronLeft } from "lucide-react"
import { useRouter } from "next/navigation"

interface PayoutItem {
  id_payout: number
  id_booking: number
  id_owner_user: string
  id_profile: string
  net_cents: number
...
const STATUS_TABS: { key: PayoutItem["status"]; label: string }[] = [
...
export default function BookingPayoutsAdminPage() {
  ...
  const [noteFor, setNoteFor] = useState<{ id: number } | null>(null)
  ...
  async function load() {
    ...
    try {
      ...
      const res = await fetch(`/api/admin/booking-payouts?${params.toString()}`, {
  ...
  async function markPaid(id_payout: number, note: string) {
    setBusyId(id_payout)
    setError(null)
    try {
      const token = getToken()
      const res = await fetch(`/api/admin/booking-payouts/${id_payout}/mark-paid`, {
        method: "POST",
      ...
      <div className="mx-auto max-w-6xl">
        ...
        <header className="mb-6 flex flex-wrap items-center justify-between gap-3">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-2xl font-bold">Agendamentos — Payouts</h1>
              <p className="text-xs text-muted-foreground">Saldo dos profissionais após holdback de 8 dias.</p>
            </div>
          </div>
        </header>
        ...
        {summary && (
          <div className="mb-6 grid grid-cols-2 gap-2 md:grid-cols-4">
            ...
            <Tile label="Total geral" value={summary.aguardando_cents + summary.aprovado_cents + summary.pago_cents} count={summary.aguardando_count + summary.aprovado_count + summary.pago_count} icon={Calendar} tone="muted" />
          </div>
        )}
        ...
        <div className="mb-4 flex flex-wrap items-center gap-2">
          ...
          <div className="ml-auto flex w-full max-w-sm items-center gap-2 rounded-full border border-border bg-card px-3 py-1.5">
            ...
            <input
              ...
              placeholder="username, email, cliente, perfil…"
            ...
            <table className="w-full text-sm">
              <thead className="bg-muted/30 text-xs uppercase tracking-wide text-muted-foreground">
                <tr>
                  ...
                  <th className="px-3 py-2 text-left">Serviço / Cliente</th>
                  <th className="px-3 py-2 text-left">Agend.</th>
              ...
              <tbody className="divide-y divide-border/70">
                {visible.map((b) => (
                  <tr key={b.id_payout} className="text-sm">
                    <td className="px-3 py-2 align-top">
                      <p className="font-medium">{b.profile_display_name || "—"}</p>
                      <p className="text-[11px] text-muted-foreground">@{b.owner_username || "—"} · {b.owner_email || "—"}</p>
                    </td>
                    <td className="px-3 py-2 align-top">
                      <p>{b.service_name || "—"}</p>
                    ...
                    <td className="px-3 py-2 align-top text-[11px] text-muted-foreground">
                      <p>Criado: {formatDate(b.created_at)}</p>
                    ...
                    <td className="px-3 py-2 align-top text-right">
                      {b.status === "aprovado" ? (
                        <button
                          type="button"
                          disabled={busyId === b.id_payout}
                          onClick={() => { setNoteFor({ id: b.id_payout }); setNoteValue("") }}
                        ...
                        >
                          {busyId === b.id_payout ? <Loader2 className="h-3 w-3 animate-spin" aria-hidden /> : <CheckCircle2 className="h-3 w-3" aria-hidden />}
                          Marcar pago
                        </button>
                      ) : b.status === "pago" ? (
