  ...
  return (
    <div className="fl-sharp mx-auto max-w-5xl px-4 pb-24 pt-6">
      ...
      <header className="border-b-4 border-current pb-4">
        ...
        <div className="mt-1 flex flex-wrap items-end justify-between gap-3">
          ...
          <button
            ...
            className="flex items-center gap-2 border-2 border-current bg-yellow-400 px-4 py-2 text-xs font-black uppercase tracking-wide text-black hover:bg-yellow-300"
          >
      ...
      <div className="mt-6 flex flex-col gap-2 sm:flex-row">
        <label className="flex flex-1 items-center gap-2 border-2 border-current px-3 py-2">
        ...
        <label className="flex items-center gap-2 border-2 border-current px-3 py-2 sm:w-56">
        ...
        <button
          onClick={() => void load()}
          className="border-2 border-current px-4 py-2 text-xs font-black uppercase hover:bg-current/10"
        >
          {t("searchCta", "Buscar")}
        </button>
      </div>
      ...
      {state === "error" && (
        <div className="mt-8 border-2 border-current p-6 text-center text-sm opacity-70">
          {t("loadError", "Erro ao carregar as academias. Tente novamente.")}
        </div>
      )}
      {state === "loaded" && academies.length === 0 && (
        <div className="mt-8 border-2 border-dashed border-current p-10 text-center">
          ...
          {academies.map((a) => (
            <li key={a.id_academy}>
              <Link
                ...
                className="block border-2 border-current p-4 transition-colors hover:bg-current/5"
              >
                <div className="flex items-start gap-3">
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <p className="mt-1 flex items-center gap-1 text-xs font-bold uppercase tracking-wide opacity-70">
                      <Users className="h-3 w-3" />
                      {String(a.member_count)} {t("membersSuffix", "vinculados")}
                    </p>
                  </div>
                </div>
                {a.descricao && <p className="mt-3 line-clamp-2 text-sm opacity-70">{a.descricao}</p>}
              </Link>
            </li>
          ))}
        </ul>
      ...
      {createOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4" onClick={() => setCreateOpen(false)}>
          <div
            className="fl-sharp max-h-[90vh] w-full max-w-lg overflow-y-auto border-4 border-current bg-background p-6"
            onClick={(e) => e.stopPropagation()}
          >
            <div className="flex items-start justify-between gap-4 border-b-2 border-current pb-3">
            ...
            <div className="mt-4 space-y-3">
              <label className="block">
                <span className="text-[11px] font-bold uppercase tracking-wide opacity-70">{t("fieldName", "Nome da academia")}</span>
                <input
                  value={form.nome}
                  onChange={(e) => setForm((f) => ({ ...f, nome: e.target.value }))}
                  className="mt-1 w-full border-2 border-current bg-transparent px-3 py-2 text-sm outline-none"
                />
              </label>
              <label className="block">
                <span className="text-[11px] font-bold uppercase tracking-wide opacity-70">{t("fieldCity", "Cidade")}</span>
                <input
                  value={form.cidade}
              ...
              <label className="block">
                <span className="text-[11px] font-bold uppercase tracking-wide opacity-70">{t("fieldDescription", "Descrição")}</span>
                <textarea
              ...
              <label className="block">
                <span className="text-[11px] font-bold uppercase tracking-wide opacity-70">{t("fieldApiUrl", "URL da API (Gym Provider)")}</span>
                <input
                  value={form.api_base_url}
                  onChange={(e) => setForm((f) => ({ ...f, api_base_url: e.target.value }))}
                  placeholder="https://crm.suaacademia.com.br"
                  className="mt-1 w-full border-2 border-current bg-transparent px-3 py-2 font-mono text-sm outline-none"
                />
              </label>
              <label className="block">
                <span className="text-[11px] font-bold uppercase tracking-wide opacity-70">{t("fieldApiToken", "Token da API")}</span>
                <input
              ...
              <p className="flex items-start gap-2 text-[11px] opacity-60">
            ...
            <div className="mt-5 flex justify-end gap-2 border-t-2 border-current pt-4">
              <button
                ...
                className="border-2 border-current px-4 py-2 text-xs font-black uppercase"
              >
                {t("cancel", "Cancelar")}
              </button>
              <button
