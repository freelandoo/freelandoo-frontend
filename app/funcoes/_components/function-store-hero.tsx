...
// Vitrine da Loja de Funções — layout/coreografia portados da home do TENKA
// (carrossel fullscreen GSAP: card central + previous/next em 3D, headline
// gigante de fundo, autoplay com barra de progresso, wheel/swipe/teclado).
// Adaptação Freelandoo: cantos retos (.fl-sharp na página), fonte display
// Anton (fl-display) e conteúdo vindo do catálogo (admin edita).
...
function ProductCard({
  ...
  return (
    ...
    >
      <div className="relative flex h-full w-full flex-col overflow-hidden bg-[#101014] shadow-[0_40px_90px_-24px_rgba(0,0,0,0.55)]">
        ...
        {/* Área de imagem — placeholder com ícone da função até o admin subir arte */}
        <div
          className="relative flex-1"
          style={{ backgroundColor: product.placeholder_color }}
        ...
        >
          ...
          {/* Etiqueta da função */}
          <span className="absolute bottom-3 left-3 bg-black/30 px-2 py-1 text-[10px] font-semibold uppercase tracking-[0.24em] text-white/60">
            {product.nav_label}
          </span>
          ...
          {owned && (
            <span
              className="absolute right-3 top-3 flex items-center gap-1 px-2 py-1 text-[10px] font-bold uppercase tracking-[0.2em]"
...
export default function FunctionStoreHero({
  ...
  const { contextSafe } = useGSAP(
    () => {
      ...
      gsap.set(containerRef.current, { backgroundColor: current.background_color })
      gsap.set(q("[data-hero-glow]"), { backgroundColor: current.accent_color })
  ...
  const runTransition = (from: number, to: number, direction: Direction) => {
      ...
      tl.to(fadeEls, { autoAlpha: 0, duration: 0.18, ease: "none", overwrite: "auto" })
        ...
        .add(() => {
          ...
          if (containerRef.current) {
            gsap.set(containerRef.current, { backgroundColor: toSlide.background_color })
          }
          gsap.set(glowEl, { backgroundColor: toSlide.accent_color })
        })
    ...
    tl.to(containerRef.current, { backgroundColor: toSlide.background_color, overwrite: "auto" }, 0)
      .to(glowEl, { backgroundColor: toSlide.accent_color, overwrite: "auto" }, 0)
  ...
  const arrowButtonClasses =
    "flex h-12 w-12 items-center justify-center border-2 border-white text-white " +
  ...
  return (
    <section
      ...
      style={{ backgroundColor: slides[0].background_color }}
    ...
    >
      ...
      <div
        ...
        style={{ backgroundColor: slides[0].accent_color }}
      />
        ...
        <div className="flex items-center justify-between px-5 py-4 sm:px-10 sm:py-6 lg:px-14">
          ...
          >
            ...
            <span className="text-xl font-bold uppercase" style={{ letterSpacing: "-0.04em" }}>
              ...
              <span aria-hidden="true" style={{ color: activeSlide.accent_color }}>
                _
              </span>
            </span>
          </Link>
            ...
            {slides.map((slide, index) => (
              <button
                ...
                className={`min-h-[44px] text-[11px] font-bold uppercase tracking-[0.28em] transition-colors duration-300 ${
                  index === activeIndex ? "text-white" : "text-white/55 hover:text-white"
                }`}
              >
                {slide.nav_label}
              </button>
      ...
      >
        <p data-hero-eyebrow className="text-[11px] font-bold uppercase tracking-[0.35em] text-white/80">
          {activeSlide.eyebrow}
        </p>
        ...
        <p data-hero-description className="mt-2 text-[15px] font-bold md:text-[17px]" style={{ color: activeSlide.accent_color }}>
          {activeOwned
            ? t("ownedLine", "Você já tem esta função")
            : `${formatPriceBRL(activeSlide.price_cents, locale)} · ${t("lifetimeShort", "pagamento único")}`}
        </p>
        ...
        <Link
          ...
          className="mt-4 inline-flex min-h-[44px] items-center gap-2 border-2 border-white/80 px-5 text-[11px] font-bold uppercase tracking-[0.14em] text-white transition-colors hover:bg-white/[0.14] md:hidden"
        >
      ...
      <div data-hero-fade className="absolute bottom-10 right-8 z-[60] hidden md:block lg:bottom-16 lg:right-14">
        <Link data-hero-cta href={`/funcoes/${activeSlide.feature_key}`} className="group block text-white">
          ...
          <span
            ...
            style={{ backgroundColor: activeSlide.accent_color }}
          />
        </Link>
      </div>
            ...
            return (
              ...
              >
                <span
                  ...
                  style={{ backgroundColor: activeSlide.accent_color, opacity: active ? 1 : 0 }}
                />
                <span
            ...
            return (
              ...
              >
                <span
                  ...
                  style={{ width: active ? 24 : 12, backgroundColor: active ? "#FFFFFF" : "rgba(255,255,255,0.4)" }}
                />
              </button>
            )
          })}
      ...
      <div aria-hidden="true" className="pointer-events-none absolute inset-x-0 bottom-0 z-[65] h-[3px] bg-white/15">
        <div data-hero-progressbar className="h-full w-full origin-left scale-x-0 bg-white/80" />
      </div>
