...
const ALLOWED = new Set(["status", "choose", "dismiss"])

async function proxy(request: Request, action: string, method: "GET" | "POST") {
  const log = apiFlow(`onboarding/intent/${action}:${method}`)
  let status = 500
  log.start(request)
  try {
    ...
    const url = `${getBackendApiUrl()}/onboarding/intent/${action}`
