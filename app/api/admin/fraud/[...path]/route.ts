...
/**
 * Proxy catch-all do Painel de Fraude (mig 201). Encaminha qualquer rota sob
 * /api/admin/fraud/* para o backend (/admin/fraud/*), preservando método,
...
async function forward(request: Request, method: string, pathParts: string[]) {
  ...
  const log = apiFlow(`admin/fraud/${sub}:${method}`)
  let status = 500
  log.start(request)
  try {
    ...
    const url = `${getBackendApiUrl()}/admin/fraud/${sub}${search}`
    ...
    log.fail(error)
    status = 500
    return Response.json({ error: "Erro no proxy de fraude" }, { status: 500 })
  } finally {
    log.end(status)
  }
}
