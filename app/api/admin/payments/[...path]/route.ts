...
/**
 * Proxy catch-all do Painel de Pagamentos (projeto PayDebug). Encaminha
 * qualquer rota sob /api/admin/payments/* para o backend (/admin/payments/*),
...
async function forward(request: Request, method: string, pathParts: string[]) {
  ...
  const log = apiFlow(`admin/payments/${sub}:${method}`)
  let status = 500
  log.start(request)
  try {
    ...
    const url = `${getBackendApiUrl()}/admin/payments/${sub}${search}`
    ...
    log.fail(error)
    status = 500
    return Response.json({ error: "Erro no proxy de pagamentos" }, { status: 500 })
  } finally {
    log.end(status)
  }
}
