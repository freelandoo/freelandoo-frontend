...
export async function GET(request: Request) {
  const log = apiFlow("admin/booking-payouts:GET")
  let status = 500
  log.start(request)
  try {
    const authHeader = request.headers.get("Authorization")
    if (!authHeader) { status = 401; return Response.json({ error: "Autorização necessária" }, { status: 401 }) }
    const url = `${backend()}/admin/booking-payouts${new URL(request.url).search}`
    ...
    return Response.json({ error: "Erro ao listar payouts de agendamento" }, { status: 500 })
