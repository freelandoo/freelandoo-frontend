...
async function proxy(request: Request, method: "GET" | "POST") {
  const log = apiFlow(`admin/affiliate:${method}`)
  let status = 500
  log.start(request)
  try {
    ...
    const url = `${BACKEND}/admin/affiliate${method === "GET" ? search || "" : ""}`
    ...
    log.fail(error)
    status = 500
    return Response.json({ error: "Erro no proxy admin/affiliate" }, { status: 500 })
  } finally {
    log.end(status)
  }
}
