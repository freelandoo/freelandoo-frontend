...
/**
 * Proxy catch-all das Lives (admin). Encaminha /api/admin/lives/* para o
 * backend (/admin/lives/*), preservando método, querystring, Authorization
 * e body JSON. Cobre a loja de presentes (gifts).
 */
async function forward(request: Request, method: string, pathParts: string[]) {
  const sub = pathParts.join("/")
  const log = apiFlow(`admin/lives/${sub}:${method}`)
  let status = 500
  log.start(request)
  try {
    ...
    const url = `${getBackendApiUrl()}/admin/lives/${sub}${search}`
