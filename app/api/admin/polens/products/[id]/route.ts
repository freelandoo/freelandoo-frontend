...
export async function PUT(request: Request, { params }: { params: Promise<{ id: string }> }) {
  const log = apiFlow("admin/polens/products/[id]:PUT")
  let status = 500
  log.start(request)
  try {
    ...
    const url = `${getBackendApiUrl()}/admin/polens/products/${encodeURIComponent(id)}`
...
export async function DELETE(request: Request, { params }: { params: Promise<{ id: string }> }) {
  const log = apiFlow("admin/polens/products/[id]:DELETE")
  let status = 500
  log.start(request)
  try {
    const auth = request.headers.get("Authorization")
