...
async function proxy(request: Request, method: "PUT" | "DELETE", id_coupon: string) {
  const log = apiFlow(`admin/coupons/${id_coupon}/discount-override:${method}`)
  let status = 500
  log.start(request)
  try {
    ...
    const url = `${BACKEND}/admin/coupons/${id_coupon}/discount-override`
    ...
    log.fail(error)
    status = 500
    return Response.json({ error: "Erro proxy discount-override" }, { status: 500 })
  } finally {
    log.end(status)
  }
}
