...
async function proxy(request: Request, method: "GET" | "POST") {
  const log = apiFlow(`admin/coupons/commission-settings:${method}`)
  let status = 500
  log.start(request)
  try {
    ...
    const url = `${BACKEND}/admin/coupons/commission-settings`
    ...
    log.fail(error)
    status = 500
    return Response.json({ error: "Erro proxy commission-settings" }, { status: 500 })
  } finally {
    log.end(status)
  }
}
