...
/**
 * Proxy catch-all dos Clusters de Live (admin). Encaminha
 * /api/admin/live-clusters/* para o backend (/admin/live-clusters/*):
 * detalhe, membros, botões, start/end e signal.
 */
async function forward(request: Request, method: string, pathParts: string[]) {
  const sub = pathParts.join("/")
  const log = apiFlow(`admin/live-clusters/${sub}:${method}`)
  let status = 500
  log.start(request)
  try {
    ...
    const url = `${getBackendApiUrl()}/admin/live-clusters/${sub}${incoming.search}`
