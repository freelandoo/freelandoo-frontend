...
export async function GET(request: Request, { params }: { params: Promise<{ id: string }> }) {
  const log = apiFlow("admin/blog/posts/[id]:GET")
  let status = 500
  log.start(request)
  try {
    ...
    const url = `${getBackendApiUrl()}/admin/blog/posts/${encodeURIComponent(id)}`
...
export async function PUT(request: Request, { params }: { params: Promise<{ id: string }> }) {
  const log = apiFlow("admin/blog/posts/[id]:PUT")
  let status = 500
  log.start(request)
  try {
    const auth = request.headers.get("Authorization")
...
export async function DELETE(request: Request, { params }: { params: Promise<{ id: string }> }) {
  const log = apiFlow("admin/blog/posts/[id]:DELETE")
  let status = 500
  log.start(request)
  try {
    const auth = request.headers.get("Authorization")
