...
export async function PUT(request: Request, { params }: { params: Promise<{ id: string }> }) {
  const log = apiFlow("admin/store/prohibited-rules/[id]:PUT")
  let status = 500
  log.start(request)
  try {
    ...
    const url = `${getBackendApiUrl()}/admin/store/prohibited-rules/${id}`
...
export async function DELETE(request: Request, { params }: { params: Promise<{ id: string }> }) {
  const log = apiFlow("admin/store/prohibited-rules/[id]:DELETE")
  let status = 500
  log.start(request)
  try {
    const { id } = await params
    ...
    log.fail(error); status = 500
    return Response.json({ error: "Erro ao remover regra" }, { status: 500 })
  } finally { log.end(status) }
}
