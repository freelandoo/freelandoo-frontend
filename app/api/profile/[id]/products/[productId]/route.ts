...
export async function PATCH(request: Request, { params }: { params: Promise<{ id: string; productId: string }> }) {
  const log = apiFlow("profile/[id]/products/[productId]:PATCH")
  let status = 500
  log.start(request)
  try {
    ...
    const url = `${backend()}/profile/${id}/products/${productId}`
...
export async function DELETE(request: Request, { params }: { params: Promise<{ id: string; productId: string }> }) {
  const log = apiFlow("profile/[id]/products/[productId]:DELETE")
  let status = 500
  log.start(request)
  try {
    const { id, productId } = await params
