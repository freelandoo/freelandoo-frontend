...
export async function GET(request: Request) {
  const log = apiFlow("me/booking-payouts:GET")
  let status = 500
  log.start(request)
  try {
    const authHeader = request.headers.get("Authorization")
    if (!authHeader) { status = 401; return Response.json({ error: "Autorização necessária" }, { status: 401 }) }
    const url = `${backend()}/me/booking-payouts${new URL(request.url).search}`
    ...
    return Response.json({ error: "Erro ao buscar saldo de agendamentos" }, { status: 500 })
