...
/**
 * Proxy catch-all do Residência. Encaminha /api/residences/* para o backend
 * (/residences/*). TUDO aqui exige Authorization: nenhuma rota de condomínio é
...
async function forward(request: Request, method: string, pathParts: string[]) {
  ...
  const log = apiFlow(`residences/${sub}:${method}`)
  let status = 500
  log.start(request)
  try {
    const auth = request.headers.get("Authorization")
    const incoming = new URL(request.url)
    const url = `${getBackendApiUrl()}/residences/${sub}${incoming.search}`
    ...
    log.fail(error)
    status = 500
    return Response.json({ error: "Erro no proxy de residências" }, { status: 500 })
  } finally {
    log.end(status)
  }
}
