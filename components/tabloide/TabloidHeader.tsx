...
/**
 * TabloidHeader — cabeçalho tabloide global para páginas fora da landing
 * (grupo `(with-footer)`: institucionais + docs legais, hoje sem header).
 *
 * Espelha o LandingHeader (canvas escuro, dourado, logo + nav central +
 * Entrar/Comece agora), mas reage ao estado de auth e usa as traduções (i18n)
 * como o SiteFooter. Auto-envolto em `.fl-root` para resolver os tokens
 * `--fl-*` (que só existem nesse escopo), já que o layout-pai é dark utilitário.
 */
    ...
    <header className="fl-root sticky top-0 z-50 w-full">
      <div className="border-b border-[#F5F1E8]/8 bg-[#15120E]/85 backdrop-blur-md supports-[backdrop-filter]:bg-[#15120E]/65">
        <div className="mx-auto flex w-full max-w-[1180px] items-center gap-4 px-5 py-3.5 sm:px-8">
          <Link href="/" className="flex shrink-0 items-center gap-2" aria-label="Freelandoo">
          ...
          <nav className="mx-auto hidden items-center gap-7 lg:flex">
            {NAV.map((n) => (
              <Link key={n.href} href={n.href} className="text-sm font-semibold text-[#F5F1E8]/75 transition hover:text-[#F5F1E8]">
                {tNav(n.key, n.label)}
              </Link>
            ))}
          </nav>
          ...
          <div className="ml-auto flex shrink-0 items-center gap-2 sm:gap-3 lg:ml-0">
            {isLoggedIn ? (
              ...
              <>
                <Link href="/login" className="hidden rounded-full border border-[#F5F1E8]/25 px-5 py-2 text-sm font-bold text-[#F5F1E8] transition hover:border-[#F5F1E8]/50 sm:inline-flex">
                  {tAuth("login", "Entrar")}
                </Link>
                <GoldButton href="/cadastro" className="px-5 py-2.5 text-sm">
                  {tNav("getStarted", "Comece agora")} <ArrowRight className="h-4 w-4" />
                </GoldButton>
              </>
            )}
            <button
        ...
        {open && (
          <div className="border-t border-[#F5F1E8]/8 bg-[#15120E] lg:hidden">
            <nav className="mx-auto flex w-full max-w-[1180px] flex-col px-5 py-3 sm:px-8">
              ...
              {!isLoggedIn && (
                <Link href="/login" onClick={() => setOpen(false)} className="py-3 text-base font-semibold text-[#F5F1E8]">
                  {tAuth("login", "Entrar")}
                </Link>
              )}
            </nav>
