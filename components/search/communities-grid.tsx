...
export function CommunitiesGrid({ machineId, regionId, q, rootRef }: Props) {
  ...
  // Condomínio (mig 196) NUNCA aparece na vitrine: é entidade de moradores,
  // encontrada em /comunidades. Aqui a listagem é fixa em kind=common.
  const query = (() => {
    const params = new URLSearchParams()
    if (machineId) params.set("id_machine", String(machineId))
    if (regionId) params.set("id_region", String(regionId))
    if (q) params.set("q", q)
    params.set("kind", "common")
